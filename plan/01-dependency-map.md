...
```
GATE 0  Aryan provisions 4 coworkers → sends keys            (5 min, blocks everything below)
   │
   ...
   └── Akshat ── catalog reading, Cara/Ravi fixtures, both prompts ───┤
                                                                       ▼
GATE 1  Adamay pushes skeleton + "types.ts frozen"                   (~2:05, blocks handler code)
   │
   ...
   └── Akshat ── Cara steps 1–3 (contacts, deal, care plan doc) ──┤   needs pipeline ids + rulebook id
                                                                  ▼
GATE 2  Adamay pushes config/ids.json with container ids            (~2:25)
   │
   ├── Adamay ── seed 150 contacts + ~70 visit series (runs ~6 min) ─┐
   ├── Adamay ── people.ts, rulebook.ts, llm.ts, approvals.ts ───────┤  PARALLEL
   ├── Aryan  ── Sam step 5–6 code (filters, ranking) against fixtures ─┤
   └── Akshat ── Cara steps 4–6, then Ravi steps 1–5 ─────────────────┤
                                                                       ▼
GATE 3  Adamay: "contacts + visits seeded"                           (~2:45, blocks anything that reads the roster)
   │
   ...
   └── Akshat ── Ravi step 6 (cert sweep) + email polish ───────┤
                                                                 ▼
GATE 4  Adamay: approvals.ts pushed                                  (~3:00, blocks Sam step 10 only)
   │
   ├── Adamay ── tunnel + webhooks + first live email into ops@ ─┐
   ├── Aryan  ── Sam steps 9–12 (YES reply, approval, updates) ──┤  PARALLEL
   └── Akshat ── live test both public forms from a phone ───────┤
                                                                  ▼
GATE 5  Full run from `npm run reset` works end to end             (~3:30)
   │
...
- `GATE 4 open: approvals.ts pushed`
- `GATE 5 open: full run passed`
- `GATE 6: FREEZE`

Step numbers refer to the lane docs: `lane-1-foundation-and-ops.md` (Adamay), `lane-2-sam-scheduler.md` (Aryan), `lane-3-cara-and-ravi.md` (Akshat).
