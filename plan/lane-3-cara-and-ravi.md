...
**Owner: Akshat.** You build `src/coworkers/cara.ts`, `src/coworkers/ravi.ts`, and `fixtures/cara/*.json`, `fixtures/ravi/*.json`. Nothing else. Read `plan/00-README-everyone.md` first, especially sections 3, 4, and 9.
...
1. Read the command shapes you will use:
   ...
   npx ambiguous@latest forms create --help        # field types and the options shape; tell Adamay if the ids in lane 1 step 4.5 need changing
   npx ambiguous@latest calendar availability --help
   ```
2. Write the fixtures (shapes from README 4.1). Use the exact field ids Adamay will create (lane 1, step 4.5):
...
When Adamay posts keys and ids: `AMBI_API_TOKEN=$AMBI_KEY_CARA npx ambiguous@latest whoami` → Cara; same for Ravi.
...
Ops may hand Cara an email instead of a form. `mail.get` → `llm.askJSON` into the same `answers` shape (fill what you can, `null` the rest) → run steps 1–6, but skip booking if `family_email` is missing and instead create a `Needs a human:` task. If you are not going to get to this by 3:15 PM, tell Adamay so Ops uses its fallback (task for the owner + short reply).
...
- Window: next three business days, 09:00–17:00, 30 minutes, first gap in the owner's availability (same gap finder as Cara; put it in your own file `src/coworkers/slots.ts`? No: shared files are Adamay's. Keep a small `findGap()` in each coworker file, or ask Adamay to add `cal.firstGap()` to `ambi.ts`. Ask early; it is 15 lines.)
...
2. Submit the real public form from a phone (the URL is in `config/ids.json`) → same result via the live server (`npm run dev` must be running on Adamay's machine; if it is not, use the fixture path and say so).
